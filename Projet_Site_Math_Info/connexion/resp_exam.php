...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    // Requête pour récupérer les infos de l'enseignant
    $sql0 = "SELECT * FROM responsables_exam WHERE matricule_user = '$idUSER'";
    ...
    if ($resultatUSER && mysqli_num_rows($resultatUSER) > 0) {
        ...
        $mailUSER = htmlspecialchars($USER['mail']);
    } else {
        // Gestion du cas où aucun utilisateur n'a été trouvé
        echo "Aucun enseignant trouvé avec l'ID spécifié.";
        exit;
    }

    // Requête pour compter le nombre d'étudiants dans la table utilisateur
    $sql = "SELECT COUNT(matetu) AS total FROM etudiants";
    $resultat = mysqli_query($conn, $sql);

    if ($resultat) {
        $ligne = mysqli_fetch_assoc($resultat);
        $totaletudiant = htmlspecialchars($ligne['total']);
    } else {
    ...
    $sql1 = "SELECT COUNT(id_ct) AS total FROM cours_td";
    $resultat = mysqli_query($conn, $sql1);

    if ($resultat) {
        $ligne = mysqli_fetch_assoc($resultat);
        $totalcours = htmlspecialchars($ligne['total']);
    } else {
        echo "Erreur lors de l'exécution de la requête : " . mysqli_error($conn);
        $totalcours = 0;
    }
    ...
    <div class="contener">
            ...
            <ul>
                <li>
                    <a href="#" class="logo1">
                        ...
                        <h3 class="nav-item">Enseignant <span><?php echo $nomUSER; ?></span></h3>
                ...
                <li>
                    <div class="element">
                        <div class="entete">
                            ...
                            <h3>Co<span>urs</span></h3>
                        </div>
                        <div class="contenue">
                            <a href="../actions/liste_cours&td.php?id=<?php echo $idUSER; ?>">
                                <span>Mes Cours</span>
                            </a>
                            <a href="enregistrer_ct.php?id=<?php echo $idUSER; ?>">
                ...
                <li>
                    <div class="element">
                        <div class="entete">
                            <i class='bx bx-book'></i>
                            <h3>Exa<span>men</span></h3>
                        </div>
                        <div class="contenue">
                            <a href="../actions/liste_PV.php?id=<?php echo $idUSER; ?>">
                                <span>PVs</span>
                            </a>
                            <a href="enregistrer_PV.php?id=<?php echo $idUSER; ?>">
                                <span>Enregistrer</span>
                            </a>
                        </div>
                    </div>
        ...
        <section class="section1">
            ...
            <div class="start">
                ...
                <div class="card">
                    <i class='bx bx-book'></i>
                    <h3><?php echo $totalcours; ?><span> Cours</span></h3>
                    <p> enregistrés</p>
                </div>
                <div class="card">
                    <i class='bx bx-happy'></i>
                    <h3>65% de nos <span>des enseignants</span></h3>
                    <p>sont des enseignant-chercheurs.</p>
                </div>
            </div>
            ...
            <section class="section2">
                <div class="recent">
                    <?php
                        ...
                        if ($resultatens && mysqli_num_rows($resultatens) > 0) {
                            ...
                            echo '<table>';
                            echo '<thead><tr><th>Nom</th><th>Prénom</th><th>Date de Naissance</th><th>Email</th><th>Groupe TD</th></tr></thead>';
                            echo '<tbody>';

                            while ($row = mysqli_fetch_assoc($resultatens)) {
                                $nometu = htmlspecialchars($row["nometu"]);
                                $prenometu = htmlspecialchars($row['prenometu']);
                                $datenaiss = htmlspecialchars($row['datnaissetu']);
                                $emailetu = htmlspecialchars($row['mailetu']);
                                $grpetu = htmlspecialchars($row["grou_td"]);
                            ...
                            echo '<p>Aucun étudiant enregistré.</p>';
                        }
                ...
                <div class="footer">
                    <a href="../actions/liste_etudiant.php?id=<?php echo $idUSER; ?>">Plus</a>
                </div>
            </section>
        </section>
    </div>
