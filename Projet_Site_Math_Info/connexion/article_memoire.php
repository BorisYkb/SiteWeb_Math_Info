...
<body>
    <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (
                ...
                $type_d = mysqli_real_escape_string($conn, $type_d);
                $annee = mysqli_real_escape_string($conn, $annee);
                $id_auteur = mysqli_real_escape_string($conn, $id_auteur);
                $id_filiere = mysqli_real_escape_string($conn, $id_filiere);
                $niveau = mysqli_real_escape_string($conn, $niveau);

                // Vérifier l'existence des id_auteur, id_filiere et niveau dans la base de données
                $queries = [
                    
                    "SELECT id_filiere FROM filieres WHERE id_filiere = '$id_filiere'",
                    "SELECT id_niveau FROM niveau WHERE id = '$niveau'"
                ];
                $valid = true;
                ...
                if ($valid) {
                    ...
                    if ($_FILES['contenu']['error'] === UPLOAD_ERR_OK) {
                        $fileContent = file_get_contents($pdfFile);

                        // Exécuter la requête d'insertion
                        $sql = "INSERT INTO documents (titre, type_d, annee, contenu, id_auteur, id_filiere, niveau) VALUES (?,?,?,?,?,?,?)";
                        $stmt = $conn->prepare($sql);
                        if ($stmt === false) {
                            $message2 = "Erreur de préparation de la requête : " . htmlspecialchars($conn->error);
                        } else {
                            $stmt->bind_param("ssssisi", $titre, $type_d, $annee, $fileContent, $id_auteur, $id_filiere, $niveau);
                        ...
                        $message2 = "Erreur lors de l'importation du fichier.";
                    }
                } else {
                    $message2 = "ID auteur, ID filière ou niveau non valide.";
                }
    ...
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post"
            ...
            <div class="form-field">
                <input type="text" name="type_d" require>
                <label for="type_d">TYPE DE DOCUMENT</label>
            </div>

            <div class="form-field">
                <input type="text" name="annee" require>
            ...
            <div class="form-field">
                <input type="number" name="id_auteur" require>
