...
<head>
    ...
    <title>Emplois du temps | MI</title>
    <style>
    ...
    <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (
                ...
                isset($_FILES['contenu']) && isset($_POST['id_filiere']) &&
                isset($_POST['annee_uni'])
            ) {
                ...
                $sql = "INSERT INTO emplois_du_temps ( date_debut, date_fin, contenu, id_filiere, annee_uni) VALUES (?,?,?,?,?)";
                $stmt = $conn->prepare($sql);
                if ($stmt === false) {
                    $message2 = "Erreur de préparation de la requête : " . htmlspecialchars($conn->error);
                } else {
                    $stmt->bind_param("sssis", $date_debut, $date_fin, $fileContent, $id_filiere, $annee_uni);
    ...
    <div class="container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post"
            ...
            <div class="form-field">
                <input type="date" name="date_debut" require>
                <label for="date_debut">DATE DE DEBUT</label>
            </div>

            <div class="form-field">
                <input type="date" name="date_fin" require>
